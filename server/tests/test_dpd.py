...
class DPDTrackerTests(unittest.TestCase):
    @patch.dict("server.dpd.os.environ", {"DPD_POSTCODE": "8004"})
    def test_uses_the_postcode_passed_for_each_parcel_not_the_environment(self):
        tracker = DPDTracker(timeout=5)
        with patch.object(
            tracker, "_api_fetch", return_value={"status": "in_transit"}
        ) as api_fetch:
            tracker.fetch(TRACKING_NUMBER, "3000")

        self.assertEqual(tracker.postcode, "")
        api_fetch.assert_called_once_with(TRACKING_NUMBER, "3000")
