...
DPD_FIREBASE_API_KEY = "AIzaSyDHMkUNUyUwFrQzKJhdC_J-L7QEwNUzwrc"  # gitleaks:allow -- public Android app identifier
...
class DPDTracker:
    def __init__(
        ...
        self.postcode = (postcode or "").strip()
    ...
    def fetch(self, tracking_number: str, postcode: str | None = None) -> dict[str, Any]:
        if not re.fullmatch(r"\d{14}", tracking_number):
            raise ValueError("DPD tracking numbers must contain 14 digits")
        resolved_postcode = self.postcode if postcode is None else postcode.strip()
        if resolved_postcode and not re.fullmatch(r"\d{4}", resolved_postcode):
            raise ValueError("DPD postcode must contain exactly 4 digits")

        api_error: Exception | None = None
        try:
            result = self._api_fetch(tracking_number, resolved_postcode)
    ...
    def _api_fetch(
        self, tracking_number: str, postcode: str | None = None
    ) -> dict[str, Any]:
        resolved_postcode = self.postcode if postcode is None else postcode.strip()
        postcode_verified: bool | None = None
        try:
            payload = self._details_with_fresh_token(
                tracking_number, resolved_postcode or None
            )
            if resolved_postcode:
                postcode_verified = True
        except _DPDAPIHTTPError as exc:
            if not resolved_postcode or exc.status != 400:
                raise
