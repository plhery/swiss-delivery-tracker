...
private struct ExperimentalParcelListView: View {
    ...
    var body: some View {
        NavigationStack(path: $path) {
            ...
            .navigationDestination(for: UUID.self) { parcelID in
                ExperimentalParcelDetailView(parcelID: parcelID, transition: parcelTransition)
            }
            .safeAreaInset(edge: .bottom, spacing: 8) { bottomControls }
        }
        .sheet(isPresented: $showingAdd) {
            ExperimentalAddParcelView(draft: sharedDraft)
...
private struct ExperimentalPassportView: View {
    @EnvironmentObject private var store: ParcelStore
    @EnvironmentObject private var localizer: Localizer
