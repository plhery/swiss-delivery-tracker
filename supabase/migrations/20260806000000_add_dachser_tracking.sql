-- Dachser Customer Iberia links are capability URLs. Store them only for
-- Dachser parcels and require the exact public-detail origin and shipment.
alter table public.packages
  drop constraint packages_tracking_url_check,
  add constraint packages_tracking_url_check
    check (
      ...
      or (
        ...
        and (
          tracking_url ~ '[?&]hash=[A-Za-z0-9_-]{4,256}([&#]|$)'
          or (
            tracking_url ~ '[?&]clave=[A-Za-z0-9_-]{4,256}([&#]|$)'
...
begin
  ...
  if normalized_url is not null and not (
    ...
    or (
      ...
      and (
        normalized_url ~ '[?&]hash=[A-Za-z0-9_-]{4,256}([&#]|$)'
        or (
          normalized_url ~ '[?&]clave=[A-Za-z0-9_-]{4,256}([&#]|$)'
