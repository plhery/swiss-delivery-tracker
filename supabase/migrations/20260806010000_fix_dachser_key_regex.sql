-- PostgreSQL's ARE engine rejects interval bounds above 255. Express the
-- supported four-to-256 character Dachser capability keys with a 255-bound
-- interval plus one optional final character.

alter table public.packages
  drop constraint if exists packages_tracking_url_check,
  add constraint packages_tracking_url_check
    check (
      ...
      or (
        ...
        and char_length(tracking_url) <= 4096
        and tracking_url ~ '^https://customeriberia[.]dachser[.]com(?::443)?/customerarea/utilidades/seguimiento-publico/detalle[?]'
        and tracking_url ~ ('[?&]numeroUnico=' || tracking_number || '([&#]|$)')
        and (
          tracking_url ~ '[?&]hash=[A-Za-z0-9_-]{4,255}[A-Za-z0-9_-]?([&#]|$)'
          or (
            tracking_url ~ '[?&]clave=[A-Za-z0-9_-]{4,255}[A-Za-z0-9_-]?([&#]|$)'
            and tracking_url ~ '[?&]fecha=[0-9]{8}([&#]|$)'
          )
        )
      )
    ) not valid;
