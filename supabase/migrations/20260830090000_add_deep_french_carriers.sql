-- Register the remaining backend-only French carrier adapters. Mondial Relay
-- reuses the historical dpd_postcode column/API parameter to avoid a breaking
-- rename; the constraint now validates it as a carrier-specific delivery
-- postcode (four digits for Swiss DPD, five for Mondial Relay).
  ...
  add constraint packages_carrier_check
    check (
      carrier in (
        ...
        'fedex', 'dpd', 'dpd-fr', 'mondial-relay', 'relais-colis',
        'la-poste', 'chronopost', 'gls-fr', 'colis-prive', 'geodis',
...
comment on column public.packages.dpd_postcode is
  'Recipient postcode for carrier verification (historical column name retained for compatibility).';
...
begin
  ...
  if (p_carrier = 'dpd' and coalesce(normalized_postcode, '') !~ '^[0-9]{4}$')
      ...
      or (p_carrier not in ('dpd', 'mondial-relay') and normalized_postcode is not null) then
